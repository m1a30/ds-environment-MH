---
title: "Raster and vector data integration"
...
Here's a visualization of the types of data we are using. 
...
This demo explains how to crop a raster using the extent of a vector
...
## In-class: Crop to Vector Points Extent
...
# cropping canopy height model to what the extent of the study plot locations is
  # CHM_HARV_Cropped <- crop(x = CHM_HARV, y = aoi_boundary_HARV)

canopy_height_cropped <- crop (x = CHM_HARV, y = plot_locations_sp_HARV)

# plotting vegetation plot location points on top of CHM
canopy_height_cropped_df <- as.data.frame(canopy_height_cropped, xy = TRUE)

ggplot() +
  geom_raster(data = canopy_height_cropped_df,
              aes(x = x, y = y, fill = HARV_chmCrop)) +
...
tree_height <- extract(x = CHM_HARV, y = aoi_boundary_HARV, raw = FALSE)
  # raw = FALSE turns it into a dataframe for us, if not it stays as spatial data
...
## In class: Extract Raster Height Values For Plot Locations
...
mean_tree_height_around_plot_locations <- extract(x = CHM_HARV,
                                  y = st_buffer(plot_locations_sp_HARV, dist = 20),
                                  fun = mean)


ggplot((data = mean_tree_height_around_plot_locations, aes(x = ID, HARV_chmCrop)) + geom_col()
```
